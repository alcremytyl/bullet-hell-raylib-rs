...
use crate::{bullet::Bullet, draw_outline, entity::Entity, Draw};
...
impl Player {
    pub fn shoot(&self) -> Vec<Bullet> {
        if let Some(w) = &self.entity.weapon {
            let res = w.shoot(&self.entity.pos, self.entity.angle);
