...
use raylib_rs::{
    bullets::Bullets, player::Player, target::Target, traits::Drawable, Bullet, BULLET_DEVIATION,
    CHUNK_SIZE, FIRE_RATE, PLAYER_SLOW, PLAYER_SPEED, SCREEN_H, SCREEN_W,
};
...
async fn main() {
    ...
    loop {
        ...
        if bullets[0].len() > 0 {
            let mut i = 0;
            let mut to_delete = vec![false; bullets.len()];
